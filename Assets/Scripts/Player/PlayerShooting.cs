...
public class PlayerShooting : NetworkBehaviour
{
    ...
    private float shootCoolDownTime = 0f; // 距离上次开枪过了多久，单位：s
    ...
    private void OnHit(Vector3 pos, Vector3 normal, HitEffectMaterial material) // 击中点的特效
    {
        ...
        particleSystem.Play();
        Destroy(hitEffectObject, 9f); // 弹孔消失时间
    }
    ...
    private void Shoot()
    {
        ...
        if (Physics.Raycast(cam.transform.position, cam.transform.forward, out hit, currentWeapon.range, mask))
        {
            ...
            {
                ShootServerRpc(hit.collider.name, currentWeapon.damage); // 调用服务器端的ShootServerRpc函数，由于服务器端没有本地玩家，可以理解为把函数发到服务器端调用
